...
export default function InstagramDownloader() {
  ...
  const handleDownload = (itag: string) => {
    ...
    const size =
      ...
      (duration > 0 ? estimateSize(duration, height, isAudio) : 5 * 1024 * 1024);

    setDownloadTitle(data.details.title);

    // Note: Since Instagram videos are already standard combined files (progressive MP4),
    // we use needsMerge=false. This pipes the download stream directly from the server stdout,
    // making the download fast and responsive.
    executeDownload(
      url,
      itag,
      data.details.title,
            ...
            <div className="md:col-span-5 flex flex-col gap-3 sm:gap-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-4 mt-3 text-xs text-zinc-400">
                  <span className="font-semibold text-pink-400 flex items-center gap-1 shrink-0">
                    {data.details.author}
                  </span>
            ...
            <div className="md:col-span-7 flex flex-col gap-4">
              <div>
                <h3 className="text-xs font-extrabold text-zinc-400 uppercase tracking-widest mb-3">
                  Select Quality to Download (Direct MP4)
                </h3>
                <QualityPreview
