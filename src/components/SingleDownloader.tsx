...
import { executeDownload, formatDuration } from "@/utils/downloader";
import UrlInput from "@/components/ui/UrlInput";
import ErrorBanner from "@/components/ui/ErrorBanner";
import DownloadProgress from "@/components/ui/DownloadProgress";

/**
 * Self-contained single-video download domain.
 * Fetches video metadata from /api/info and delegates streaming to
 * the executeDownload utility.
 */
export default function SingleDownloader() {
  ...
  const handleDownload = (itag: string) => {
    ...
    let size = format?.contentLength ?? undefined;
    ...
    if (!size) {
      if (data.details.duration && data.details.duration > 0) {
        ...
        const isHigh = label.includes("720p") || label.includes("1080p") || label.includes("1440p") || label.includes("2160p");
        const isMedium = label.includes("480p");
        const bps = isAudio ? 128 * 1000 : isHigh ? 1500 * 1000 : isMedium ? 800 * 1000 : 400 * 1000;
        size = (data.details.duration * bps) / 8;
      } else {
        size = 10 * 1024 * 1024; // 10 MB fallback
      }
    }

    executeDownload(url, itag, data.details.title, itag, (state) => {
  ...
  return (
    <div className="w-full flex flex-col items-center">
      ...
      {/* Error */}
      {error && (
        <ErrorBanner title="Failed to Load Content" message={error} />
      )}
              ...
              <div className="flex flex-col gap-4">
                ...
                <div>
                  <h4 className="text-xs font-extrabold text-zinc-400 uppercase tracking-widest mb-2.5 flex items-center gap-1.5">
                    ...
                    Recommended Formats (Video + Audio)
                  </h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    {data.formats.filter((f) => f.hasVideo && f.hasAudio).length === 0 && (
                ...
                <div className="mt-2">
                  <h4 className="text-xs font-extrabold text-zinc-400 uppercase tracking-widest mb-2.5 flex items-center gap-1.5">
                    ...
                    Audio Only (MP3 / High Quality M4A)
                  </h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {data.formats
                      .filter((f) => !f.hasVideo && f.hasAudio)
                      ...
                      .map((format) => (
                        ...
                        >
                          <div>
                            ...
                            <span className="text-[10px] text-zinc-400">
                              {format.mimeType.includes("m4a") ? "m4a (preferred)" : "mp3 / webm"} •{" "}
                              {format.contentLength
                                ? `${(format.contentLength / (1024 * 1024)).toFixed(1)} MB`
                                : "Unknown Size"}
                            </span>
                ...
                <div className="mt-2">
                  <details className="group border border-white/5 rounded-xl bg-white/[0.01]">
                    <summary className="flex items-center justify-between p-3.5 text-xs font-bold text-zinc-400 uppercase tracking-widest cursor-pointer hover:bg-white/[0.03]">
                      <span className="flex items-center gap-1.5">
                        <Layers className="w-3.5 h-3.5 text-brand-blue" />
                        Advanced Adaptive Formats (Video Only)
                      </span>
